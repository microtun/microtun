...
const CONFIG_KIND: &str = "TrackerConfig";
...
pub struct ServerOptions {
    ...
    pub mtu: u16,
    /// Whether the Tracker forwards authenticated type-5 relay packets between peers.
...
struct Draft {
    /// Local alias from `Peer.Name`; the Tracker record is unnamed.
    name: Option<String>,
    public_key: [u8; 32],
...
pub fn parse(text: &str, path: &Path) -> Result<Loaded, ConfigError> {
    ...
    // The configured prefix belongs to the Tracker's virtual interface, but the
    // Tracker peer itself owns only its host address. Keeping those concepts
    // separate prevents e.g. 10.0.0.1/24 from publishing 10.0.0.0/24 as an
    // address owned by the Tracker. IPv6 gets the equivalent /128 host route.
...
fn validate_base_config(config: &TrackerConfig, path: &Path) -> Result<(), ConfigError> {
    ...
    if decode_key(&config.tunnel.private_key).is_err() {
        return Err(ConfigError::file(
            path,
            "Tunnel.PrivateKey must be a 32-byte tunnel key in standard base64",
        ));
    }
...
fn validate_names(config: &TrackerConfig, path: &Path) -> Result<(), ConfigError> {
...
fn self_public_key(drafts: &[Draft]) -> [u8; 32] {
    drafts
        ...
        .expect("a parsed configuration has exactly one Tracker draft")
        .public_key
}
