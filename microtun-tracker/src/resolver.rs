//! Local peer resolver backed by the Tracker configuration file.
//!
...
impl FixedServer {
    pub fn from_loaded(loaded: &Loaded) -> Result<Self, String> {
        if loaded
            ...
            .is_none()
        {
            return Err("tracker record is missing from its own registry".to_string());
        }
        Ok(Self {
...
/// Project a served registry record into the Tracker's own tunnel view.
///
/// A peer may name the Tracker's public key as its `Relay`, telling remote
...
mod tests {
    ...
    fn server_local_resolver_strips_self_relay() {
        ...
        // Tracker as the relay.
        assert_eq!(
            loaded.registry.lookup_key(&[0xAA; 32]).unwrap().relay,
            Some(server_public())
        );

        // The Tracker's own tunnel is the relay, so its local view must be
