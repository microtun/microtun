...
//! turns those into TCP streams, and the RPC layer serves the Peers API
...
///
/// The Peers API's largest possible message is `RECORD_MESSAGE_LEN` (1 KiB), and
...
///
/// RX additionally has to hold one opening handshake, which is the largest
/// single thing a client ever sends: a browser's `GET` carries `Origin`,
/// `User-Agent`, and whatever cookies the page has, and
/// `SERVER_HANDSHAKE_LEN` (2 KiB) is the ceiling the handshake reader
/// enforces on it.
    ...
    fn new(
        ...
        iface.update_ip_addrs(|addrs| {
            for address in local_addresses {
                ...
                addrs.push(cidr).expect("tracker addresses should fit");
            }
        });
...
mod tests {
    ...
    /// A simulated remote tunnel protocol peer opening TCP connections to the server.
    struct SimPeer {
        key: [u8; 32],
        ip: Ipv4Addr,
        port: u16,
