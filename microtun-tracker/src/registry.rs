...
pub struct PeerRecord {
    ...
    /// The key in the tunnel protocol's base64, which is the form it is served and
...
impl PeerRecord {
    ...
    pub fn info_with_endpoint(&self, endpoint: Option<SocketAddr>) -> PeerInfo {
        ...
        .expect("validated Tracker records fit the bounded API type")
    }
...
impl SharedRegistry {
    ...
    /// `peer.watch` handler may also insert its keyed subscription while this
    ...
    /// not wake it until `peer.watch` indexes a key.
...
impl Registry {
    ...
    pub fn build(records: Vec<PeerRecord>) -> Result<Self, String> {
        ...
        // The Tracker is allocator-backed through `microtun-std`, so the
