...
//!
//! The Tracker terminates the tunnel itself. It binds a UDP socket
//! (`ListenPort`), runs the protocol engine, and serves the Peers API over a virtual
...
//! attributable to a peer whose tunnel session was authenticated first, and
...
struct Args {
    ...
    #[arg(value_name = "CONFIG", env = "TRACKER_CONFIG")]
    config: PathBuf,
}
...
async fn serve(config_path: &std::path::Path, loaded: config::Loaded) -> Result<(), String> {
    ...
    let server_address = registry
        ...
        .ok_or_else(|| "tracker record is missing from its own registry".to_string())?
        .address;
