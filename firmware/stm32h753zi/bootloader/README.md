# STM32H753ZI first-stage bootloader

This is the immutable recovery stage for the STM32H753ZI firmware. The current
board profile is the original NUCLEO-H753ZI. It uses `embassy-boot-stm32` to
swap the ACTIVE and DFU partitions power-fail-safely. A candidate application
remains a trial until the application calls `BlockingFirmwareState::mark_booted()`.
Any reset before that confirmation causes the next bootloader pass to restore
the previously confirmed ACTIVE image.
