...
async fn main(spawner: Spawner) -> ! {
    ...
    let temperature_sensor =
        TemperatureSensor::new(peripherals.TSENS, TemperatureSensorConfig::default())
            .expect("initialize ESP32-C3 temperature sensor");
    ...
    let (identify_led, user_button) = board::io!(peripherals);
    ...
    let wifi = config
        .wifi
        .as_ref()
        .expect("ESP32-C3 device configuration requires a Wi-Fi section");
...
fn esp32_device_identity() -> DeviceIdentity {
    // The factory eFuse base MAC is the ESP32-C3's stable per-chip identifier,
...
fn esp_reset_reason_name(reason: Option<SocResetReason>) -> &'static str {
    match reason {
        ...
        Some(SocResetReason::CoreUsbUart | SocResetReason::CoreUsbJtag) => "external",
        Some(
            SocResetReason::CoreEfuseCrc
            | SocResetReason::SysClkGlitch
            | SocResetReason::CorePwrGlitch,
        ) => "hardware",
        None => "unknown",
    }
}
