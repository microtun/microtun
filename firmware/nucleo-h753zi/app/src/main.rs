...
// B1 is a plain user button rather than a boot strap, so the default reset policy applies.
...
/// Monitor the active-high B1 USER button while the device is configured.
///
/// Holding B1 erases the configuration sector and resets; the next boot finds no valid record and
/// comes up in setup mode. The debounce and hold logic is shared — all this board
/// contributes is that B1 reads high while held (UM2407, default solder-bridge routing).
#[embassy_executor::task]
async fn user_button_reset_task(button: Input<'static>, storage: &'static Storage) -> ! {
    monitor_reset(button, USER_BUTTON, storage, reset).await
}
...
/// The one user LED reserved by the firmware for `identify`.
///
/// LD1 is the active-high PB0 LED. LD2/PE1 and LD3/PB14 are deliberately left unconfigured so
/// application code can use those pins independently.
...
async fn main(spawner: Spawner) -> ! {
    ...
    // Reserve only LD1/PB0 for `identify`. LD2/PE1 and LD3/PB14 remain untouched for the rest
    // of the application.
    let identify_led = IdentifyLedPin {
        led: Output::new(p.PB0, Level::Low, Speed::Low),
    };
    // B1/PC13 is reserved only for the configuration-reset gesture: hold it for three seconds on
    // a configured device to erase the stored configuration and return to setup mode.
