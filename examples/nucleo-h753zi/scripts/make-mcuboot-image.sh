...
# Reproducibly build the NUCLEO-H753ZI payload in Docker, then create an
# MCUboot image using a public key retrieved from the network signer.
#
...
if [[ $# -eq 1 ]]; then
    ...
    OUTPUT="$CALLER_DIR/microtun-nucleo-h753zi-$VERSION.mcuboot.bin"
fi
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/microtun-nucleo-h753zi-docker.XXXXXX")"
...
printf 'Building NUCLEO-H753ZI firmware payload in Docker...\n'
