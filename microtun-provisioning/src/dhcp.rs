//! Tiny DHCP server for the provisioning-mode subnet.
//!
//! This intentionally implements only the DHCP messages needed to put a host
//! interface on the isolated provisioning link: DISCOVER/OFFER and
//! REQUEST/ACK (plus RELEASE cleanup). It advertises no router and no DNS
//! server, because provisioning mode is a local-only link.
...
const DHCP_LEASE_CAPACITY: usize =
    (PROVISION_DHCP_RANGE_END[3] - PROVISION_DHCP_RANGE_START[3] + 1) as usize;
...
/// Run the provisioning-mode DHCP server until the task is cancelled or the
/// UDP socket fails.
///
/// The pool is deliberately tiny and bounded. A lease table is kept only for
/// the lifetime of provisioning mode; rebooting the device forgets it, which is
...
pub async fn run(stack: Stack<'static>) -> Result<(), ()> {
    ...
    loop {
        ...
        // Provisioning mode is intentionally link-local. Do not service DHCP
        ...
        if let Some(server_identifier) = request.server_identifier {
            if server_identifier != PROVISION_DEVICE_IPV4 {
...
fn build_reply(request: &Request, reply: &mut [u8], message_type: u8, yiaddr: [u8; 4]) -> usize {
    ...
    if message_type == DHCP_OFFER || message_type == DHCP_ACK {
        ...
        push_option(
            ...
            &mut offset,
            OPTION_SUBNET_MASK,
            &prefix_mask(PROVISION_IPV4_PREFIX_LEN),
        );
        push_option(
            reply,
            &mut offset,
...
fn decline_lease(leases: &mut [Lease], client: ClientKey) {
    if let Some(slot) = find_client_lease(leases, client) {
        // Keep the declined address out of the pool for this provisioning-mode
...
fn lease_ip(slot: usize) -> [u8; 4] {
    [
        ...
        PROVISION_DHCP_RANGE_START[3] + slot as u8,
    ]
}

fn lease_slot(ip: [u8; 4]) -> Option<usize> {
    if ip[0..3] != PROVISION_DHCP_RANGE_START[0..3]
        || ip[3] < PROVISION_DHCP_RANGE_START[3]
        || ip[3] > PROVISION_DHCP_RANGE_END[3]
    {
        return None;
    }
    Some((ip[3] - PROVISION_DHCP_RANGE_START[3]) as usize)
}
